...
export class WaitlistService {
  ...
  private static validateEmail(email: string): { isValid: boolean; sanitized: string } {
  ...
  static async addEmail(email: string): Promise<{ 
    ...
    try {
      ...
      try {
        ...
        if (insertError?.code === '23505' || insertError?.constraint === 'waitlist_entries_email_unique') {
          console.log('📧 Email already exists (unique constraint):', sanitized)
  ...
  static async getStats(): Promise<WaitlistStats> {
    ...
    try {
      ...
      const result = await db.execute(sql`SELECT COUNT(*) as count FROM waitlist_entries`)
