...
function validateRedisUrl(url: string): boolean {
  try {
    ...
    if (
      ...
      console.warn(
        "Warning: Redis connection is not using TLS/SSL in production for external Redis",
      );
    }

    if (parsed.protocol !== "rediss:" && isLocal) {
      console.log(
        "Using local Redis connection without TLS (this is fine for local/containerized setups)",
      );
    }
